...
static int print_set_comm_info(int num, struct tax_sys_comm_item *comm_info)
{
    ...
    tmp = frame->comm_items[num].item_monney;
    memset(tmp, 0, MAX_NUM_LENGTH);
...
static int print_invoice(struct tax_sys_invoice_detail_record * inv_detail)
{
    ...
    frame->comm_num = inv_detail->item_num;
...
struct print_ops flat_print_ops = {
    .print_boot_check = print_boot_check,
    .print_invoice = print_invoice,
    .print_prepare = print_check,  
};

struct printer_type flat_printer_type = {
    .id = FLAT_PRINT_TYPE,
    .state = PRINT_UP,
    .name = "FP-650K",
    .print_ops = &flat_print_ops,
};

struct printer_type * get_flat_printer(void) 
{
    return &flat_printer_type;
}
